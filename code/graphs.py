...
def plot_college(index):
    ...
    try:
        ...
        print("DONE: "+str(i) + ": "+str(colleges.iloc[i,0]))
    except ValueError:
        errors.append(index)
        print("FAILED: "+str(i) + ": "+str(colleges.iloc[i,0]))
        os.rmdir(os.getcwd()+f'/data/college_graphs/{college_no_spaces}')

bad_graphs = pd.read_csv(os.getcwd()+f'/data/bad_graphs.csv')
indexes = list(bad_graphs['college_index'])
#plot all colleges
for i in indexes:
    #print("starting " + str(i) + ": "+str(colleges.iloc[i,0]))
    plot_college(i)
# error_df = pd.DataFrame({'college_index' : errors})
# error_df.to_csv(os.getcwd()+'/data/bad_graphs.csv')